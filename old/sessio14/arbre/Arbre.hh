...
public:
  ...
  // especificaci— operacions pœbliques
     ...
     el p.i. Žs buit */
  {
    if (primer_node!=NULL and fe.primer_node==NULL
      ...
      if (&fe != &fd) {       
        ...
        throw PRO2Excepcio 
	  ("Els dos parˆmetres de fills no poden coincidir");
    }
    else if (primer_node==NULL)
      throw PRO2Excepcio ("Un arbre buit no tŽ fills");
    else   
      throw PRO2Excepcio 
        ("Els dos parˆmetres de fills han de ser buits a la crida");
  }
  ...
  /* Post: el resultat Žs l'arrel del p.i. */
  {
    if (primer_node!=NULL)
      ...
      throw PRO2Excepcio ("Un arbre buit no tŽ arrel");
  }
  ...
  /* Post: a Žs com A per˜ havent substitu•t les fulles que contenien x
     per l'arbre as */
  {
  ...
  /* Post: els nodes de la jerarquia de nodes que comena al node apuntat per n
     tals que el seu camp info valia x i no tenien segŸents han estat substituits
  ...
  /* Pre: cert */
  /* Post: el resultat es el nombre de nodes descendents de n, ell mateix incl˜s */
  {
    int t;
    if (n == NULL) t=0;
