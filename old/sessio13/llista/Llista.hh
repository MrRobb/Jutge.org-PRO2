...
static node_llista* copia_node_llista(node_llista* m, node_llista* oact,
    ...
    /* Pre: cert */
    /* Post: si m es NULL, el resultat, u i a son NULL; en cas
       contrari, el resultat apunta al primer node d'una cadena de
       nodes que son copia de la cadena que te el node apuntat per m
       com a primer, u apunta a l'ultim node, a es o be NULL si oact
       no apunta a cap node de la cadena que comenca amb m o be apunta
...
public:
  ...
  void inici()
    /* Pre: cert */
    /* Post: el punt d'interès del p.i. esta situat a sobre del primer element de la llista
  ...
  void invertir ()
    /* Pre: cert */ 
    /* Post: el p.i. te els mateixos elements que a l'inici pero amb l'ordre invertit
       i el seu punt d'interes no s'ha mogut */
  {
    node_llista* n = primer_node;
    while (n != ultim_node) {
      /* Inv: els nodes anteriors al que apunta n en la cadena que comenca a primer_node
	 han intercanviat els seus punters a l'anterior i al seguent node */
